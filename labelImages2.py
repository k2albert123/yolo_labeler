...
def rotate_image_keep_all(image, angle_degrees: float, return_transform: bool = False):
    h, w = image.shape[:2]
    center = (w / 2.0, h / 2.0)
...
class MainWindow(QMainWindow):
    ...
    def open_folder(self):
        ...
        # Keep labels in an easy-to-find location beside the selected image folder.
        # Example: <chosen_folder>/../labels
    ...
    def rotate_current_image(self, angle_degrees: float):
        ...
        rotated, matrix, new_w, new_h = rotate_image_keep_all(
            image,
            angle_degrees,
        ...
        for box in self.boxes:
            rotated_box = self.rotate_box_with_matrix(box, matrix, new_w, new_h)
            if rotated_box is None:
                continue
            transformed_boxes.append(rotated_box)

        self.current_image = rotated
        self.current_image_h, self.current_image_w = rotated.shape[:2]
        ...
        self.canvas.set_image(QPixmap.fromImage(qimage.copy()))
        ...
        self.canvas.update_scaled_pixmap()
        self.save_labels()
        self.update_status(extra=f"Image rotated by {angle_degrees}° and boxes were realigned.")
    ...
    def rotate_box_with_matrix(
        ...
        new_xs = [p[0] for p in rotated_points]
        new_ys = [p[1] for p in rotated_points]
        rotated_box = Box(
            box.class_id,
            max(0, min(new_xs)),
            max(0, min(new_ys)),
            min(img_w - 1, max(new_xs)),
            min(img_h - 1, max(new_ys)),
        )

        if rotated_box.is_too_small():
            return None
        return rotated_box
    ...
    def save_labels(self):
        ...
        if not label_path:
            self.update_status(extra="Save skipped: no label path.")
            return
        ...
        try:
            ...
            if not lines and os.path.exists(label_path) and os.path.getsize(label_path) > 0:
                reply = QMessageBox.question(
                    self,
                    "Save Empty Labels?",
                    "There are currently 0 boxes on this image.\n\n"
                    "Saving now will clear the existing label file.\n\n"
                    "Do you want to continue?",
                    QMessageBox.Yes | QMessageBox.No,
                    QMessageBox.No,
                )
                if reply != QMessageBox.Yes:
                    self.update_status(extra="Save cancelled: existing labels kept.")
                    return
            ...
            self.update_status(
                extra=f"Saved {len(lines)} box(es) to {label_path}"
            )
