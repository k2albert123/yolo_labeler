...
Features:
- Student-friendly image folder picker
- Draw YOLO boxes
...
- Save YOLO labels
- Remove poor/unwanted images safely
- macOS/PySide6 safe dialogs
"""
...
class MainWindow(QMainWindow):
    ...
    def open_folder(self):
        ...
        selected_folder_name = os.path.basename(folder).lower()

        if selected_folder_name == "images":
    ...
    def remove_current_image(self):
        ...
        try:
            ...
            if os.path.exists(image_path):
                image_destination = unique_destination_path(
                    os.path.join(removed_images_dir, image_name)
                )
                shutil.move(image_path, image_destination)

            if label_path and os.path.exists(label_path):
                label_destination = unique_destination_path(
                    os.path.join(removed_labels_dir, os.path.basename(label_path))
                )
